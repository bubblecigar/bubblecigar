...
//外部資源：store.js，請參考：https://github.com/marcuswestin/store.js#user-content-list-of-all-plugins
...
function initiate(){
	...
	if (store.get('todoList') == undefined) {
		store.set('todoList',[]);
	}
...
function make_task_box_new(data_template){
	...
	node_tree.setAttribute("draggable","true");
...
//    (c). 呼叫upload_store(): 將整個頁面中的所有資料上傳至store 
//    (d). 呼叫arrange_order(): 整理store中的資料排序 
...
function activate_buttons(){
	...
	for (var i = document.querySelectorAll(".edit_task").length - 1; i >= 0; i--) {
		...
		document.querySelectorAll(".file_info")[i].addEventListener("dblclick", function(e){
			...
			task_box.querySelector(".file_info").setAttribute("style","display:none");
		},true);
	}
}

function activate_drag_drop(){
	var box_ar = document.querySelectorAll(".edit_task");
	var dragged_node;

	for (var i = box_ar.length - 1; i >= 0; i--) {
		box_ar[i].addEventListener("dragstart",function(e){
			dragged_node = e.target;
		},false);

		box_ar[i].addEventListener("dragover",function(e){
			e.preventDefault();
			// notifying effect
		},false);

		box_ar[i].addEventListener("dragleave",function(e){
			//cancel notifying effect
		},false);		

		box_ar[i].addEventListener("drop",function(e){
			e.preventDefault();
			//cancel notifying effect
			//move the node
			//refresh the page
			var rel_node = find_root(e);
			document.querySelector(".content").insertBefore(dragged_node,rel_node);
			upload_store();
			arrange_order();
			refresh_page();
		},false)
...
function find_root(e){
	if (e.target.className.split(" ").includes("edit_task")) {
		return e.target;
	}else if(e.target.parentNode.className.split(" ").includes("edit_task")) {
		return e.target.parentNode;
	}else if(e.target.parentNode.parentNode.className.split(" ").includes("edit_task")){
		return e.target.parentNode.parentNode;
	}else if(e.target.parentNode.parentNode.parentNode.className.split(" ").includes("edit_task")) {
...
// upload_store():
// 1. 將每個.edit_task .data_str中所儲存的textContent上傳至store的"todoList"
function upload_store(){
	var all_data = [];
	for (var i = document.querySelectorAll(".edit_task").length - 1; i >= 0; i--) {
		all_data.push(JSON.parse(document.querySelectorAll(".data_storage")[i].textContent));
	}
	// store.setItem("todoList", JSON.stringify(all_data));
	store.set('todoList',all_data);
}

// arrange_order():
// 排列store中"todoList"的陣列資料順序
function arrange_order(){
	
	var data_array = store.get('todoList');
	var finished_tasks = [];
	var highlight_tasks = [];
	var regular_tasks = [];
